...
class Converter:
    def __init__(self, root='/home/ananguye/Data/phucnah/waymo2/'):
        self.root = root
    ...
    def apply_labels(self, row):
        ...
        l_tensor1 = tf.reshape(tf.convert_to_tensor(row['[LiDARComponent].range_image_return1.values']), shape=row['[LiDARComponent].range_image_return1.shape'])
        l_tensor2 = tf.reshape(tf.convert_to_tensor(row['[LiDARComponent].range_image_return2.values']), shape=row['[LiDARComponent].range_image_return2.shape'])

        l_mask1 = l_tensor1[..., 0] > 0
        l_mask2 = l_tensor2[..., 0] > 0

        #print(segments[0].values)
        s_tensor1 = tf.reshape(tf.convert_to_tensor(row['[LiDARSegmentationLabelComponent].range_image_return1.values']), shape=row['[LiDARSegmentationLabelComponent].range_image_return1.shape'])
        s_tensor2 = tf.reshape(tf.convert_to_tensor(row['[LiDARSegmentationLabelComponent].range_image_return2.values']), shape=row['[LiDARSegmentationLabelComponent].range_image_return2.shape'])
